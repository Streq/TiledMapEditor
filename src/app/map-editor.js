define(["helper/dom",
	...
	function (Dom,
		...
		class Editor {
			...
			static instantiate(editor, pos, worldUnitSize, tileIndex){
			...
			deleteAt(pos){
				...
				ArrayUtils.removeIf(this.map,(e,index)=>{
					return (
						   e.x + e.w > t.x
						&& e.y + e.h > t.y 
						&& t.x + s   > e.x
			...
			start(root) {
                ...
                gridSize.addEventListener('change', function () {
				    editor.grid.size = parseInt(this.value);
				});
